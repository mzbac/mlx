...
void eval(const std::vector<array>& outputs, bool retain_graph /* = false */) {
  ...
  while (!tape.empty()) {
    ...
    // Skip this node if its been evaluated
    if (g.is_evaled()) {
      // Detach this node if its been evaluated and
      // we don't want the graph anymore
      if (!retain_graph) {
        g.detach();
      }
      continue;
    ...
    if (g.primitive().device() == Device::gpu) {
      ...
      scheduler::enqueue(
          stream,
          metal::make_task(
              std::move(g), std::move(g_deps), std::move(p), retain_graph));
    } else {
      auto task = [retain_graph,
        ...
        g.primitive().eval_cpu(g.inputs(), g.outputs());
        if (!retain_graph) {
          g.detach();
        }
        if (p) {
